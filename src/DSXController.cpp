...
namespace DSX {
    ...
    bool NetworkManager::SendPacket(const Packet& packet)
	{
		...
		// Check if length exceeds INT_MAX to avoid truncation
		if (jsonData.length() > static_cast<size_t>(INT_MAX)) {
			logger::error("Packet length exceeds INT_MAX: {} bytes", jsonData.length());
			return false;
		}

		// Cast length to int safely
